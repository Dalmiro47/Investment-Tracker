...
**Firestore quota gotcha (learned the hard way, 2026-09-18):** the project is on the Spark plan (50k reads / 20k writes per day, reset at midnight Pacific). `8 RESOURCE_EXHAUSTED: Quota exceeded` names the *next* Firestore call after the quota ran out, not the culprit — do not debug the line in the stack trace. `kraken_logs` holds one doc per funding event (thousands per year): never attach an unbounded `onSnapshot`/`getDocs` to it; always bound by `date` (`useKrakenTaxData` requires a `since` date and does not subscribe without one). Still unfixed: Phase 1.5 of `syncKrakenFutures` re-reads every CLOSED position plus the funding logs of every historical session and rewrites 3 docs per closed position on EVERY sync, which also makes every listener re-read them.
