...
const SUPPORTED_CURRENCIES = ['USD', 'JPY', 'BGN', 'CZK', 'DKK', 'GBP', 'HUF', 'PLN', 'RON', 'SEK', 'CHF', 'ISK', 'NOK', 'HRK', 'RUB', 'TRY', 'AUD', 'BRL', 'CAD', 'CNY', 'HKD', 'IDR', 'ILS', 'INR', 'KRW', 'MXN', 'MYR', 'NZD', 'PHP', 'SGD', 'THB', 'ZAR'];

export async function fetchECBMonthlyEUR(sinceISO: string): Promise<FXRatePoint[]> {
    const startDate = parseISO(sinceISO);
    const endDate = new Date();
    
    // Generate all month-end dates for the interval
    const monthEnds = eachMonthOfInterval({ start: startDate, end: endDate }).map(d => endOfMonth(d));

    const fxPoints: FXRatePoint[] = [];

    // api.exchangerate.host seems to have issues with 'timeseries' for older dates.
    // Fetching promises in parallel for each month-end date is more reliable.
