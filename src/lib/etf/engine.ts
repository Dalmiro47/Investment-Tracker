...
export function simulatePlan(
  ...
  for (const monthKey of months) {
    ...
    if (adminFixedPerMonth > 0) {
      adminYtdFixed += adminFixedPerMonth;
    }
    ...
    // Invest this month
    const plannedContribution = dec(getContributionForMonth(plan, monthKey));
    ...
    if (isDecember || isLastMonth) {
      ...
      if (ytdTotal > 0) {
        const preValue = components.reduce((sum, c) => {
            const px = priceNowByEtf[c.id] ?? dec(0);
            return add(sum, mul(unitsByEtf[c.id], px));
        }, dec(0));
        
        const payFromNav = Math.min(Number(preValue), ytdTotal);
        if (payFromNav > 0 && Number(preValue) > 0) {
            const ratio = payFromNav / Number(preValue);
    ...
    // ----- 3) After sells/buys, compute end-of-month portfolioValue -----
