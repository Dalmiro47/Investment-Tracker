...
export function TaxSettingsDialog({ isOpen, onOpenChange, currentSettings, onSave }: TaxSettingsDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      {/* Dynamic sizing based on active view */}
      <DialogContent className={view === 'info' ? "w-[96vw] max-w-3xl p-0 h-[80vh] flex flex-col" : "sm:max-w-lg"}>
        ...
        {view === 'info' && (
           <>
            <DialogHeader className="px-6 pt-6 pb-2 shrink-0">
               <div className="flex items-center gap-2">
                  <Button variant="ghost" size="icon" onClick={() => setView('settings')} className="-ml-2">
